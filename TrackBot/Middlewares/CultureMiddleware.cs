...
namespace TrackBot.Middlewares
{
    public class CultureMiddleware
    {
            ...
            if (context.Request.Method == "POST" && context.Request.ContentLength > 0)
            {
                using (var reader = new StreamReader(context.Request.Body, leaveOpen: true))
                {
                    ...
                    if (update?.Message?.Chat != null)
                    {
                        ...
                        if (user != null)
                        {
                            var languageCode = user.LanguageCode.ToString();
        ...
        private async Task<UserLanguagePreference> GetUserByUserId(long userId)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                var userLanguagePreferenceRepository = scope.ServiceProvider.GetRequiredService<IUserLanguagePreferenceRepository>();

                return await userLanguagePreferenceRepository.GetByUserId(userId);
            }
        }
